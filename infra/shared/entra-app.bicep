...
// Use provided serviceManagementReference, or generate a deterministic GUID.
// The Graph API rejects null and empty string, so a value is always required.
// Non-enforcing tenants accept any valid GUID; enforcing tenants (e.g., Microsoft corp)
// require a real service catalog entry — users must provide their own via env var.
var effectiveSmr = !empty(serviceManagementReference) ? serviceManagementReference : guid(appUniqueName, subscription().subscriptionId)

// App registration
resource appRegistration 'Microsoft.Graph/applications@v1.0' = {
  uniqueName: appUniqueName
  displayName: appDisplayName
  serviceManagementReference: effectiveSmr
...
resource federatedIdentityCredential 'Microsoft.Graph/applications/federatedIdentityCredentials@v1.0' = if (!empty(managedIdentityClientId) && !empty(managedIdentityPrincipalId)) {
  name: '${appUniqueName}/mcp-function-managed-identity'
  audiences: [
...
@description('The identifier URI of the application')
